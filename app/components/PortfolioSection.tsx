...
export default function PortfolioSection({ projects, title = 'Portofolio Proyek', viewText = 'Lihat Proyek' }: { projects: PortfolioItem[]; title?: string; viewText?: string }) {
  return (
    <section className="py-16 md:py-24 bg-white dark:bg-black px-4 md:px-16" id="portfolio">
      ...
      >
        {projects.map((project) => (
          <motion.div key={project._id} variants={item} className="bg-gray-50 dark:bg-gray-900 rounded-xl shadow-lg overflow-hidden flex flex-col hover:scale-[1.03] transition-transform">
              {project.image?.src ? (
                <Image
